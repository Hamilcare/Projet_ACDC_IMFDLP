...
public class Node implements INode, Runnable {
	...
	public HashMap<String, ArrayList<File>> doublons(File fileRoot) throws IOException {
		...
		for (File file : files) {
			if (file.isDirectory() && file.list().length > 0) {
				...
				if (!getMd5Table().containsKey(md5)) {
					listFile = new ArrayList();
					listFile.add(file);
					getMd5Table().put(md5, listFile);
				} else {
					getMd5Table().get(md5).add(file);
				}
			}
		}
		return getMd5Table();
	...
	public void setMd5Table(HashMap<String, ArrayList<File>> md5Table) {
		this.md5Table = md5Table;
	}
}